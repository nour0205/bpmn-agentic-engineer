...
class BpmnPlanExecutor:
    ...
    def _update_diagram(
        ...
        for operation in operations:
            ...
            plane = self._plane_containing_shape(root, str(parameters["source_ref"]))
            if plane is None:
                plane = self._plane_containing_shape(root, str(parameters["target_ref"]))
            if plane is None:
                plane = self._plane_for_process(root, str(parameters["process_id"]))
            if plane is not None:
                self._add_sequence_flow_edge(plane, root, parameters)

    def _plane_containing_shape(
        self,
        root: ET.Element,
        bpmn_element_id: str,
    ) -> ET.Element | None:
        for plane in root.iter():
            if local_name(plane.tag) != "BPMNPlane":
                continue

            for child in plane:
                if (
                    local_name(child.tag) == "BPMNShape"
                    and child.attrib.get("bpmnElement") == bpmn_element_id
                ):
                    return plane

        return None

    def _plane_for_process(
        self,
        root: ET.Element,
        process_id: str,
    ) -> ET.Element | None:
        ...
        if participant_id:
            plane = self._plane_containing_shape(root, participant_id)
            if plane is not None:
                return plane

        process = self._find_process(root, process_id)
        process_ids = {
        ...
        for plane in root.iter():
            ...
            if any(
                local_name(child.tag) == "BPMNShape"
                and child.attrib.get("bpmnElement") in process_ids
                for child in plane
            ):
                return plane
        return None
    ...
    def _add_task_shape(
        ...
        anchor = parameters.get("diagram_anchor")

        if isinstance(anchor, dict):
            plane = self._plane_for_process(root, process_id)
            if plane is None:
                return
            target_x = float(anchor.get("x", 0.0))
            target_y = float(anchor.get("y", 0.0))
            target_width = float(anchor.get("width", 155.0))
            target_height = float(anchor.get("height", 60.0))
            new_x = target_x
        else:
            if not target_id:
                return
            plane = self._plane_containing_shape(root, str(target_id))
            if plane is None:
                return
            target_shape = self._shape_for(root, str(target_id))
