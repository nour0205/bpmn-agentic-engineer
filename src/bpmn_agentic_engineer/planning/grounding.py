...
def normalize_target_label_query(value: str | None) -> str:
    """Remove a leading BPMN-kind descriptor without imposing a type filter.

    Business users commonly call a callActivity a ``sous-processus``.  The
    descriptor is not part of the visible label and must not lower the label
    match score or constrain the concrete BPMN anchor type.
    """
...
class ElementGrounder:
    ...
    def ground(
        ...
        *,
        ...
        label_query = normalize_target_label_query(target_query)
        raw_matches = self.inspector.find_elements(label_query, limit=limit)
        filtered_matches: list[dict] = []
        normalized_lane = normalize_text(lane_name)

        for match in raw_matches:
            excluded_by_process = process_id and match.get("process_id") != process_id
            excluded_by_lane = (
                lane_name and normalize_text(match.get("lane_name")) != normalized_lane
            )
            if excluded_by_process or excluded_by_lane:
                continue
        ...
        candidates = tuple(
            self._candidate(
                ...
                exact_name_match=(
                    normalize_text(match.get("name")) == label_query
                ),
            )
