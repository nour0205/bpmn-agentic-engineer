...
mcp = MCPServer("BPMN Engineering Read-Only")
...
@mcp.tool(
    title="Get BPMN element context",
    annotations=READ_ONLY,
)
...
def plan_bpmn_change(
    ...
    request: Annotated[
        ...
        Field(
            min_length=1,
            description=(
                "Natural-language change request, for example: Ajouter une tâche "
                "\"Valider le dossier\" après \"Créer le dossier\"."
            ),
        ),
    ],
    ...
    process_id: Annotated[
        str | None,
        Field(description="Optional process ID used to disambiguate process variants."),
    ] = None,
    new_name: Annotated[
        str | None,
    ...
    lane_name: Annotated[
        str | None,
        Field(description="Optional actor/lane filter used during grounding."),
    ] = None,
) -> dict:
    """Ground a natural-language request and produce a read-only atomic change plan.

    This tool never edits the BPMN file. A plan with status ``ready_for_approval``
    must still be approved before a future write-capable tool can execute it.
    """
